# Generated by the gRPC Python protocol compiler plugin. DO NOT EDIT!
"""
Mã gRPC sinh tự động từ `common/csdlpt.proto`.

Cung cấp Stub, Servicer và helper đăng ký service cho CoordinatorService và AggregatorService. Không sửa logic bằng tay; hãy sửa `.proto` rồi sinh lại.
"""
import grpc
import warnings

from common import csdlpt_pb2 as common_dot_csdlpt__pb2

GRPC_GENERATED_VERSION = '1.80.0'
GRPC_VERSION = grpc.__version__
_version_not_supported = False

try:
    from grpc._utilities import first_version_is_lower
    _version_not_supported = first_version_is_lower(GRPC_VERSION, GRPC_GENERATED_VERSION)
except ImportError:
    _version_not_supported = True

if _version_not_supported:
    raise RuntimeError(
        f'The grpc package installed is at version {GRPC_VERSION},'
        + ' but the generated code in common/csdlpt_pb2_grpc.py depends on'
        + f' grpcio>={GRPC_GENERATED_VERSION}.'
        + f' Please upgrade your grpc module to grpcio>={GRPC_GENERATED_VERSION}'
        + f' or downgrade your generated code using grpcio-tools<={GRPC_VERSION}.'
    )


class CoordinatorServiceStub(object):
    """Lớp `CoordinatorServiceStub` là client stub gRPC sinh tự động để gọi service tương ứng.
    
    Ghi chú gốc:
    Missing associated documentation comment in .proto file.
    """

    def __init__(self, channel):
        """Khởi tạo `CoordinatorServiceStub` và gắn các RPC method vào channel gRPC.
        
        Ghi chú gốc:
        Constructor.
        
                Args:
                    channel: A grpc.Channel.
        """
        self.WorkerHeartbeat = channel.unary_unary(
                '/csdlpt.CoordinatorService/WorkerHeartbeat',
                request_serializer=common_dot_csdlpt__pb2.WorkerHeartbeatMsg.SerializeToString,
                response_deserializer=common_dot_csdlpt__pb2.EmptyReply.FromString,
                _registered_method=True)
        self.WorkerPing = channel.unary_unary(
                '/csdlpt.CoordinatorService/WorkerPing',
                request_serializer=common_dot_csdlpt__pb2.WorkerPingMsg.SerializeToString,
                response_deserializer=common_dot_csdlpt__pb2.EmptyReply.FromString,
                _registered_method=True)
        self.IngestorHeartbeat = channel.unary_unary(
                '/csdlpt.CoordinatorService/IngestorHeartbeat',
                request_serializer=common_dot_csdlpt__pb2.IngestorHeartbeatMsg.SerializeToString,
                response_deserializer=common_dot_csdlpt__pb2.EmptyReply.FromString,
                _registered_method=True)
        self.GetGlobalState = channel.unary_unary(
                '/csdlpt.CoordinatorService/GetGlobalState',
                request_serializer=common_dot_csdlpt__pb2.StateRequest.SerializeToString,
                response_deserializer=common_dot_csdlpt__pb2.StateReply.FromString,
                _registered_method=True)
        self.RaftVote = channel.unary_unary(
                '/csdlpt.CoordinatorService/RaftVote',
                request_serializer=common_dot_csdlpt__pb2.RaftVoteMsg.SerializeToString,
                response_deserializer=common_dot_csdlpt__pb2.RaftVoteReply.FromString,
                _registered_method=True)
        self.RaftState = channel.unary_unary(
                '/csdlpt.CoordinatorService/RaftState',
                request_serializer=common_dot_csdlpt__pb2.RaftStateMsg.SerializeToString,
                response_deserializer=common_dot_csdlpt__pb2.EmptyReply.FromString,
                _registered_method=True)
        self.ZkVote = channel.unary_unary(
                '/csdlpt.CoordinatorService/ZkVote',
                request_serializer=common_dot_csdlpt__pb2.ZkVoteMsg.SerializeToString,
                response_deserializer=common_dot_csdlpt__pb2.ZkVoteReply.FromString,
                _registered_method=True)
        self.ZkState = channel.unary_unary(
                '/csdlpt.CoordinatorService/ZkState',
                request_serializer=common_dot_csdlpt__pb2.ZkStateMsg.SerializeToString,
                response_deserializer=common_dot_csdlpt__pb2.EmptyReply.FromString,
                _registered_method=True)


class CoordinatorServiceServicer(object):
    """Lớp `CoordinatorServiceServicer` là base servicer gRPC sinh tự động để server override các RPC.
    
    Ghi chú gốc:
    Missing associated documentation comment in .proto file.
    """

    def WorkerHeartbeat(self, request, context):
        """RPC `WorkerHeartbeat` mặc định chưa triển khai; server thật sẽ override phương thức này.
        
        Ghi chú gốc:
        Worker -> Coordinator heartbeat
        """
        context.set_code(grpc.StatusCode.UNIMPLEMENTED)
        context.set_details('Method not implemented!')
        raise NotImplementedError('Method not implemented!')

    def WorkerPing(self, request, context):
        """RPC `WorkerPing` mặc định chưa triển khai; server thật sẽ override phương thức này.
        
        Ghi chú gốc:
        Worker -> Coordinator ping for Liveness
        """
        context.set_code(grpc.StatusCode.UNIMPLEMENTED)
        context.set_details('Method not implemented!')
        raise NotImplementedError('Method not implemented!')

    def IngestorHeartbeat(self, request, context):
        """RPC `IngestorHeartbeat` mặc định chưa triển khai; server thật sẽ override phương thức này.
        
        Ghi chú gốc:
        Ingestor -> Coordinator heartbeat
        """
        context.set_code(grpc.StatusCode.UNIMPLEMENTED)
        context.set_details('Method not implemented!')
        raise NotImplementedError('Method not implemented!')

    def GetGlobalState(self, request, context):
        """RPC `GetGlobalState` mặc định chưa triển khai; server thật sẽ override phương thức này.
        
        Ghi chú gốc:
        Worker -> Coordinator state pull
        """
        context.set_code(grpc.StatusCode.UNIMPLEMENTED)
        context.set_details('Method not implemented!')
        raise NotImplementedError('Method not implemented!')

    def RaftVote(self, request, context):
        """RPC `RaftVote` mặc định chưa triển khai; server thật sẽ override phương thức này.
        
        Ghi chú gốc:
        Raft HA voting
        """
        context.set_code(grpc.StatusCode.UNIMPLEMENTED)
        context.set_details('Method not implemented!')
        raise NotImplementedError('Method not implemented!')

    def RaftState(self, request, context):
        """RPC `RaftState` mặc định chưa triển khai; server thật sẽ override phương thức này.
        
        Ghi chú gốc:
        Raft HA state replication
        """
        context.set_code(grpc.StatusCode.UNIMPLEMENTED)
        context.set_details('Method not implemented!')
        raise NotImplementedError('Method not implemented!')

    def ZkVote(self, request, context):
        """RPC `ZkVote` mặc định chưa triển khai; server thật sẽ override phương thức này.
        
        Ghi chú gốc:
        Simulated ZK HA voting
        """
        context.set_code(grpc.StatusCode.UNIMPLEMENTED)
        context.set_details('Method not implemented!')
        raise NotImplementedError('Method not implemented!')

    def ZkState(self, request, context):
        """RPC `ZkState` mặc định chưa triển khai; server thật sẽ override phương thức này.
        
        Ghi chú gốc:
        Simulated ZK HA state replication
        """
        context.set_code(grpc.StatusCode.UNIMPLEMENTED)
        context.set_details('Method not implemented!')
        raise NotImplementedError('Method not implemented!')


def add_CoordinatorServiceServicer_to_server(servicer, server):
    """Đăng ký `add_CoordinatorServiceServicer_to_server` vào gRPC server bằng các method handler đã sinh tự động."""
    rpc_method_handlers = {
            'WorkerHeartbeat': grpc.unary_unary_rpc_method_handler(
                    servicer.WorkerHeartbeat,
                    request_deserializer=common_dot_csdlpt__pb2.WorkerHeartbeatMsg.FromString,
                    response_serializer=common_dot_csdlpt__pb2.EmptyReply.SerializeToString,
            ),
            'WorkerPing': grpc.unary_unary_rpc_method_handler(
                    servicer.WorkerPing,
                    request_deserializer=common_dot_csdlpt__pb2.WorkerPingMsg.FromString,
                    response_serializer=common_dot_csdlpt__pb2.EmptyReply.SerializeToString,
            ),
            'IngestorHeartbeat': grpc.unary_unary_rpc_method_handler(
                    servicer.IngestorHeartbeat,
                    request_deserializer=common_dot_csdlpt__pb2.IngestorHeartbeatMsg.FromString,
                    response_serializer=common_dot_csdlpt__pb2.EmptyReply.SerializeToString,
            ),
            'GetGlobalState': grpc.unary_unary_rpc_method_handler(
                    servicer.GetGlobalState,
                    request_deserializer=common_dot_csdlpt__pb2.StateRequest.FromString,
                    response_serializer=common_dot_csdlpt__pb2.StateReply.SerializeToString,
            ),
            'RaftVote': grpc.unary_unary_rpc_method_handler(
                    servicer.RaftVote,
                    request_deserializer=common_dot_csdlpt__pb2.RaftVoteMsg.FromString,
                    response_serializer=common_dot_csdlpt__pb2.RaftVoteReply.SerializeToString,
            ),
            'RaftState': grpc.unary_unary_rpc_method_handler(
                    servicer.RaftState,
                    request_deserializer=common_dot_csdlpt__pb2.RaftStateMsg.FromString,
                    response_serializer=common_dot_csdlpt__pb2.EmptyReply.SerializeToString,
            ),
            'ZkVote': grpc.unary_unary_rpc_method_handler(
                    servicer.ZkVote,
                    request_deserializer=common_dot_csdlpt__pb2.ZkVoteMsg.FromString,
                    response_serializer=common_dot_csdlpt__pb2.ZkVoteReply.SerializeToString,
            ),
            'ZkState': grpc.unary_unary_rpc_method_handler(
                    servicer.ZkState,
                    request_deserializer=common_dot_csdlpt__pb2.ZkStateMsg.FromString,
                    response_serializer=common_dot_csdlpt__pb2.EmptyReply.SerializeToString,
            ),
    }
    generic_handler = grpc.method_handlers_generic_handler(
            'csdlpt.CoordinatorService', rpc_method_handlers)
    server.add_generic_rpc_handlers((generic_handler,))
    server.add_registered_method_handlers('csdlpt.CoordinatorService', rpc_method_handlers)


 # This class is part of an EXPERIMENTAL API.
class CoordinatorService(object):
    """Lớp `CoordinatorService` cung cấp helper gRPC experimental sinh tự động cho service tương ứng.
    
    Ghi chú gốc:
    Missing associated documentation comment in .proto file.
    """

    @staticmethod
    def WorkerHeartbeat(request,
            target,
            options=(),
            channel_credentials=None,
            call_credentials=None,
            insecure=False,
            compression=None,
            wait_for_ready=None,
            timeout=None,
            metadata=None):
        """Gọi RPC `WorkerHeartbeat` của `CoordinatorService` qua gRPC experimental API."""
        return grpc.experimental.unary_unary(
            request,
            target,
            '/csdlpt.CoordinatorService/WorkerHeartbeat',
            common_dot_csdlpt__pb2.WorkerHeartbeatMsg.SerializeToString,
            common_dot_csdlpt__pb2.EmptyReply.FromString,
            options,
            channel_credentials,
            insecure,
            call_credentials,
            compression,
            wait_for_ready,
            timeout,
            metadata,
            _registered_method=True)

    @staticmethod
    def WorkerPing(request,
            target,
            options=(),
            channel_credentials=None,
            call_credentials=None,
            insecure=False,
            compression=None,
            wait_for_ready=None,
            timeout=None,
            metadata=None):
        """Gọi RPC `WorkerPing` của `CoordinatorService` qua gRPC experimental API."""
        return grpc.experimental.unary_unary(
            request,
            target,
            '/csdlpt.CoordinatorService/WorkerPing',
            common_dot_csdlpt__pb2.WorkerPingMsg.SerializeToString,
            common_dot_csdlpt__pb2.EmptyReply.FromString,
            options,
            channel_credentials,
            insecure,
            call_credentials,
            compression,
            wait_for_ready,
            timeout,
            metadata,
            _registered_method=True)

    @staticmethod
    def IngestorHeartbeat(request,
            target,
            options=(),
            channel_credentials=None,
            call_credentials=None,
            insecure=False,
            compression=None,
            wait_for_ready=None,
            timeout=None,
            metadata=None):
        """Gọi RPC `IngestorHeartbeat` của `CoordinatorService` qua gRPC experimental API."""
        return grpc.experimental.unary_unary(
            request,
            target,
            '/csdlpt.CoordinatorService/IngestorHeartbeat',
            common_dot_csdlpt__pb2.IngestorHeartbeatMsg.SerializeToString,
            common_dot_csdlpt__pb2.EmptyReply.FromString,
            options,
            channel_credentials,
            insecure,
            call_credentials,
            compression,
            wait_for_ready,
            timeout,
            metadata,
            _registered_method=True)

    @staticmethod
    def GetGlobalState(request,
            target,
            options=(),
            channel_credentials=None,
            call_credentials=None,
            insecure=False,
            compression=None,
            wait_for_ready=None,
            timeout=None,
            metadata=None):
        """Gọi RPC `GetGlobalState` của `CoordinatorService` qua gRPC experimental API."""
        return grpc.experimental.unary_unary(
            request,
            target,
            '/csdlpt.CoordinatorService/GetGlobalState',
            common_dot_csdlpt__pb2.StateRequest.SerializeToString,
            common_dot_csdlpt__pb2.StateReply.FromString,
            options,
            channel_credentials,
            insecure,
            call_credentials,
            compression,
            wait_for_ready,
            timeout,
            metadata,
            _registered_method=True)

    @staticmethod
    def RaftVote(request,
            target,
            options=(),
            channel_credentials=None,
            call_credentials=None,
            insecure=False,
            compression=None,
            wait_for_ready=None,
            timeout=None,
            metadata=None):
        """Gọi RPC `RaftVote` của `CoordinatorService` qua gRPC experimental API."""
        return grpc.experimental.unary_unary(
            request,
            target,
            '/csdlpt.CoordinatorService/RaftVote',
            common_dot_csdlpt__pb2.RaftVoteMsg.SerializeToString,
            common_dot_csdlpt__pb2.RaftVoteReply.FromString,
            options,
            channel_credentials,
            insecure,
            call_credentials,
            compression,
            wait_for_ready,
            timeout,
            metadata,
            _registered_method=True)

    @staticmethod
    def RaftState(request,
            target,
            options=(),
            channel_credentials=None,
            call_credentials=None,
            insecure=False,
            compression=None,
            wait_for_ready=None,
            timeout=None,
            metadata=None):
        """Gọi RPC `RaftState` của `CoordinatorService` qua gRPC experimental API."""
        return grpc.experimental.unary_unary(
            request,
            target,
            '/csdlpt.CoordinatorService/RaftState',
            common_dot_csdlpt__pb2.RaftStateMsg.SerializeToString,
            common_dot_csdlpt__pb2.EmptyReply.FromString,
            options,
            channel_credentials,
            insecure,
            call_credentials,
            compression,
            wait_for_ready,
            timeout,
            metadata,
            _registered_method=True)

    @staticmethod
    def ZkVote(request,
            target,
            options=(),
            channel_credentials=None,
            call_credentials=None,
            insecure=False,
            compression=None,
            wait_for_ready=None,
            timeout=None,
            metadata=None):
        """Gọi RPC `ZkVote` của `CoordinatorService` qua gRPC experimental API."""
        return grpc.experimental.unary_unary(
            request,
            target,
            '/csdlpt.CoordinatorService/ZkVote',
            common_dot_csdlpt__pb2.ZkVoteMsg.SerializeToString,
            common_dot_csdlpt__pb2.ZkVoteReply.FromString,
            options,
            channel_credentials,
            insecure,
            call_credentials,
            compression,
            wait_for_ready,
            timeout,
            metadata,
            _registered_method=True)

    @staticmethod
    def ZkState(request,
            target,
            options=(),
            channel_credentials=None,
            call_credentials=None,
            insecure=False,
            compression=None,
            wait_for_ready=None,
            timeout=None,
            metadata=None):
        """Gọi RPC `ZkState` của `CoordinatorService` qua gRPC experimental API."""
        return grpc.experimental.unary_unary(
            request,
            target,
            '/csdlpt.CoordinatorService/ZkState',
            common_dot_csdlpt__pb2.ZkStateMsg.SerializeToString,
            common_dot_csdlpt__pb2.EmptyReply.FromString,
            options,
            channel_credentials,
            insecure,
            call_credentials,
            compression,
            wait_for_ready,
            timeout,
            metadata,
            _registered_method=True)


class AggregatorServiceStub(object):
    """Lớp `AggregatorServiceStub` là client stub gRPC sinh tự động để gọi service tương ứng.
    
    Ghi chú gốc:
    Missing associated documentation comment in .proto file.
    """

    def __init__(self, channel):
        """Khởi tạo `AggregatorServiceStub` và gắn các RPC method vào channel gRPC.
        
        Ghi chú gốc:
        Constructor.
        
                Args:
                    channel: A grpc.Channel.
        """
        self.SendWorkerWatermark = channel.unary_unary(
                '/csdlpt.AggregatorService/SendWorkerWatermark',
                request_serializer=common_dot_csdlpt__pb2.WorkerWatermarkMsg.SerializeToString,
                response_deserializer=common_dot_csdlpt__pb2.EmptyReply.FromString,
                _registered_method=True)


class AggregatorServiceServicer(object):
    """Lớp `AggregatorServiceServicer` là base servicer gRPC sinh tự động để server override các RPC.
    
    Ghi chú gốc:
    Missing associated documentation comment in .proto file.
    """

    def SendWorkerWatermark(self, request, context):
        """RPC `SendWorkerWatermark` mặc định chưa triển khai; server thật sẽ override phương thức này.
        
        Ghi chú gốc:
        Heuristic Worker -> Aggregator watermark report
        """
        context.set_code(grpc.StatusCode.UNIMPLEMENTED)
        context.set_details('Method not implemented!')
        raise NotImplementedError('Method not implemented!')


def add_AggregatorServiceServicer_to_server(servicer, server):
    """Đăng ký `add_AggregatorServiceServicer_to_server` vào gRPC server bằng các method handler đã sinh tự động."""
    rpc_method_handlers = {
            'SendWorkerWatermark': grpc.unary_unary_rpc_method_handler(
                    servicer.SendWorkerWatermark,
                    request_deserializer=common_dot_csdlpt__pb2.WorkerWatermarkMsg.FromString,
                    response_serializer=common_dot_csdlpt__pb2.EmptyReply.SerializeToString,
            ),
    }
    generic_handler = grpc.method_handlers_generic_handler(
            'csdlpt.AggregatorService', rpc_method_handlers)
    server.add_generic_rpc_handlers((generic_handler,))
    server.add_registered_method_handlers('csdlpt.AggregatorService', rpc_method_handlers)


 # This class is part of an EXPERIMENTAL API.
class AggregatorService(object):
    """Lớp `AggregatorService` cung cấp helper gRPC experimental sinh tự động cho service tương ứng.
    
    Ghi chú gốc:
    Missing associated documentation comment in .proto file.
    """

    @staticmethod
    def SendWorkerWatermark(request,
            target,
            options=(),
            channel_credentials=None,
            call_credentials=None,
            insecure=False,
            compression=None,
            wait_for_ready=None,
            timeout=None,
            metadata=None):
        """Gọi RPC `SendWorkerWatermark` của `AggregatorService` qua gRPC experimental API."""
        return grpc.experimental.unary_unary(
            request,
            target,
            '/csdlpt.AggregatorService/SendWorkerWatermark',
            common_dot_csdlpt__pb2.WorkerWatermarkMsg.SerializeToString,
            common_dot_csdlpt__pb2.EmptyReply.FromString,
            options,
            channel_credentials,
            insecure,
            call_credentials,
            compression,
            wait_for_ready,
            timeout,
            metadata,
            _registered_method=True)
