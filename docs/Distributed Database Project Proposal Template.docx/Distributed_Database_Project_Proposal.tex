...
\begin{itemize}[leftmargin=*]
    ...
    \begin{itemize}[label={-}]
        ...
        \begin{equation}
            W_{heur}(i, p) = \max_{j < i} T_{event\_max} - Q_p(\{\ell\})
        \end{equation}
...
\begin{itemize}[leftmargin=*]
    ...
    \begin{equation}
        \ell = T_{\text{arrival}} - T_{\text{event}} = \frac{\text{dropoff\_timestamp} - \text{pickup\_timestamp}}{60} = \frac{\text{trip\_duration}}{60}
    \end{equation}
    ...
    \begin{equation}
        PartitionID = \text{hash}(host) \bmod 12
    \end{equation}
...
\texttt{IDLE\_TIMEOUT\_S} & Ngưỡng đánh dấu phân mảnh nhàn rỗi tường minh (\texttt{TEMPORARY\_IDLE}). & 2.0 s \\
...
\begin{table}[H]
    ...
    \begin{tabular}{p{0.16\textwidth} p{0.18\textwidth} p{0.27\textwidth} p{0.27\textwidth}}
        ...
        Raft replication & Leader $\leftrightarrow$ Followers & gRPC \texttt{RaftState} / \texttt{RaftVote} & Sao chép \texttt{W\_global}, \texttt{partition\_assignment}, \texttt{failover\_history}, \texttt{term}; commit khi đa số (2/3) xác nhận. \\
...
\begin{itemize}[leftmargin=*]
    ...
    \item \textbf{Active-Standby Control Plane}: Aggregator được duy trì qua cặp dự phòng nóng thông qua khóa phân tán trên ZooKeeper. Aggregator Primary nhận thông tin DDSketch từ các Worker để tổng hợp phân phối độ trễ toàn cục, điều phối Watermark thích ứng mà không làm tắc nghẽn dòng chảy dữ liệu. Các bản ghi muộn sẽ được định tuyến tự động vào hàng đợi đền bù (DLQ).
\end{itemize}
...
\begin{table}[H]
    ...
    \begin{tabular}{p{0.16\textwidth} p{0.18\textwidth} p{0.27\textwidth} p{0.27\textwidth}}
        ...
        HA Active--Standby & Primary $\leftrightarrow$ Standby & Khóa ZooKeeper/file lock + Shared Volume & Standby đồng bộ nóng metadata; Primary mất kết nối $>$ 3s $\Rightarrow$ Standby tiếp quản, RTO $\le$ 2s. \\
...
\begin{itemize}[leftmargin=*]
    \item \textbf{Ingestor Heartbeat}: Ingestor gửi thông tin tiến độ đọc dữ liệu và độ lệch đồng hồ vật lý (\texttt{clock skew}) tới Coordinator (Strict) hoặc Aggregator (Heuristic).
...
\begin{table}[H]
    ...
    \begin{tabular}{p{0.17\textwidth} p{0.20\textwidth} p{0.24\textwidth} p{0.27\textwidth}}
        ...
        \textbf{Luồng giao tiếp} & \textbf{Hướng} & \textbf{Giao thức / Cổng} & \textbf{Nội dung \& Tần suất} \\
        \midrule
        Ingestor Heartbeat & Ingestor $\to$ Coordinator / Aggregator & gRPC \texttt{IngestorHeartbeat} (HTTP+50), fallback HTTP \texttt{POST /ingestor-heartbeat} & \texttt{ingestor\_id}, \texttt{T\_commit}, \texttt{partitions\_assigned}, \texttt{ingestor\_clock}, \texttt{offsets}; giám sát health \& clock skew. \\
...
\begin{itemize}[label={-}]
    ...
    \begin{equation}
        W_{global}(t) = \max\Big(W_{global}^{prev},\ \min_{P_k \notin \text{Idle}} LW_i(P_k)\Big)
    \end{equation}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=\textwidth,height=11.0cm,keepaspectratio]{Empty Punctuation.pdf}
...
\begin{align}
    T_{\text{event}} &= \frac{T_{\text{pickup}} - T_{\text{pickup,min}}}{\text{DIV}} \\
    T_{\text{arrival}} &= \frac{T_{\text{dropoff}} - T_{\text{pickup,min}}}{\text{DIV}}
\end{align}
Theo đó, độ trễ đến muộn của mỗi sự kiện được bảo toàn nguyên vẹn về mặt tỷ lệ vật lý và chỉ bị co giãn theo hệ số nén:
\begin{equation}
    \text{Lateness} = T_{\text{arrival}} - T_{\text{event}} = \frac{T_{\text{dropoff}} - T_{\text{pickup}}}{\text{DIV}}
\end{equation}
...
\begin{table}[H]
    ...
    \begin{tabular}{rcc ccc}
        ...
        \textbf{Wait (s)}
          ...
          & \textbf{Eventual\,\%} \\
        \midrule
        0    & \textbf{8.75}  & 0.500 & 11.63 & 64.21 & \textbf{100.00} \\
        1    & \textbf{12.88} & 0.500 & 11.63 & 64.21 & \textbf{100.00} \\
        2    & \textbf{17.86} & 0.500 & 11.63 & 64.21 & \textbf{100.00} \\
        3    & \textbf{23.29} & 0.500 & 11.63 & 64.21 & \textbf{100.00} \\
        5    & \textbf{34.66} & 0.500 & 11.63 & 64.21 & \textbf{100.00} \\
        7    & \textbf{45.48} & 0.500 & 11.63 & 64.21 & \textbf{100.00} \\
        10   & \textbf{59.17} & 0.500 & 11.63 & 64.21 & \textbf{100.00} \\
        15   & \textbf{75.03} & 0.500 & 11.63 & 64.21 & \textbf{100.00} \\
        20   & \textbf{84.44} & 0.750 & 18.67 & 81.26 & \textbf{100.00} \\
        30   & \textbf{93.23} & 0.900 & 28.80 & 91.31 & \textbf{100.00} \\
        40   & \textbf{96.72} & 0.950 & 37.78 & 94.77 & \textbf{100.00} \\
        50   & \textbf{98.41} & 0.990 & 59.70 & 97.70 & \textbf{100.00} \\
        60   & \textbf{99.27} & 0.990 & 59.70 & 97.70 & \textbf{100.00} \\
        90   & \textbf{99.89} & 0.999 & 95.03 & 98.38 & \textbf{100.00} \\
        120  & \textbf{99.97} & 0.999 & 95.03 & 98.38 & \textbf{100.00} \\
        \bottomrule
    \end{tabular}}
\end{table}
