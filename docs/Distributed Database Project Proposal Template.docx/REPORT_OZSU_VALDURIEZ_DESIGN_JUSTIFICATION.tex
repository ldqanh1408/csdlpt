...
\texttt{IDLE\_TIMEOUT\_S} & Ngưỡng thời gian đánh dấu phân mảnh nhàn rỗi tường minh (\texttt{TEMPORARY\_IDLE}). & 30.0 s \\
...
\begin{itemize}
    \item \textbf{Strict Coordinator (Raft/ZK HA)}: Được triển khai thông qua cụm 3 Coordinator chịu lỗi. Hệ thống hỗ trợ bầu chọn Leader qua (1) thuật toán đồng thuận Raft nhúng (Embedded Raft) với nhân bản log và bầu cử theo nhiệm kỳ, hoặc (2) ZooKeeper Leader Election (khi cấu hình \texttt{ZK\_ENSEMBLE}) tranh chấp khóa phân tán tại \texttt{/csdlpt/coordinator-lock} và ghi Leader ID vào node tạm \texttt{/csdlpt/coordinator-leader}. Các chỉ thị điều phối kèm cặp \texttt{(term, command\_id)} làm token fencing ngăn chặn split-brain khi phân mảnh mạng.
    \item \textbf{Heuristic Aggregator (ZooKeeper/File Lock HA)}: Triển khai thông qua cơ chế dự phòng nóng (Active-Standby) được điều phối qua khóa phân tán của ZooKeeper tại \texttt{/csdlpt/aggregator-lock} hoặc File Lock dùng chung (\texttt{FileLockLeader} dùng \texttt{fcntl}/\texttt{msvcrt}). Active ghi file nhịp tim mỗi 1s. Nếu nhịp tim mất hoặc quá hạn ($>1.5$\,s), Standby tự động chiếm khóa, khôi phục trạng thái từ RocksDB/JSON và lên Active ($RTO\le 2$\,s), tối ưu hiệu năng và tính tự trị cục bộ (Site Autonomy).
\end{itemize}
