...
\begin{itemize}[leftmargin=*]
    ...
    \item \textbf{Control Plane (gRPC/HTTP)}: Strict dùng Coordinator Raft 3-node; Heuristic dùng Aggregator HA Active--Standby.
...
\begin{itemize}[leftmargin=*]
    ...
    \begin{equation}
    W_{global}(t) = \max\Big(W_{global}(t{-}1),\ \min_{\forall P_k \in \text{Active}} LW_i(P_k)\Big)
    \end{equation}
    Phân vùng phản hồi chậm (STALE) và nhàn rỗi ngầm (IDLE) \emph{vẫn} tham gia $\min()$ để giữ cam kết 0\% loss. Chỉ phân vùng xác nhận lỗi (FAILED) hoặc được Worker đánh dấu nhàn rỗi tường minh (\texttt{is\_temporary\_idle}) sau \texttt{IDLE\_TIMEOUT\_S}$=2.0$s mới bị loại trừ; khi có sự kiện mới, phân vùng tự động quay lại hàm tính --- đây là cơ chế \textbf{Idleness Bypass} chống Straggler.
...
\begin{itemize}[leftmargin=*]
    ...
    \begin{equation}
    W_{h,i}^{P_k}(t) = \max\Big(W_{h,i}^{P_k}(t{-}1),\ T_{event\_max} - L_{eff}(t)\Big),\quad L_{eff}=\text{DDSketch.quantile}(p)
    \end{equation}
    ...
    \item \textbf{Aggregator HA (Active--Standby)}: Khóa loại trừ (ephemeral node) trên ZooKeeper. Primary tính $W_{global\_h}=\min(W_{h})$ trên phân vùng active và phát mỗi 500ms; mất kết nối $>3000$ms thì Standby (đồng bộ nóng qua Shared Volume) tiếp quản, $RTO\le 2$s. Không dùng Raft để tránh chi phí đồng thuận chặn --- đánh đổi có chủ đích theo nguyên lý giảm Communication Cost của Özsu \& Valduriez.
