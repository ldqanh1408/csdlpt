...
$\delta$ & Biên an toàn (Wait Time) cố định của chế độ Strict. & 10 s (mặc định) \\
...
$term$ & Nhiệm kỳ (term) gắn vào Fencing Token để chống Split-brain. & số nguyên \\
...
\begin{itemize}
    \item \textbf{Cơ chế Bỏ qua phân mảnh nhàn rỗi (Idleness Bypass)}: Khi một phân mảnh tạm thời không phát sinh dữ liệu mới vượt quá ngưỡng thời gian quy định (mặc định 30 giây), Worker sẽ tự động đánh dấu phân mảnh đó là nhàn rỗi tường minh (explicit idle) và báo cáo trong heartbeat. Coordinator Leader khi nhận được nhịp tim sẽ tự động loại trừ phân mảnh nhàn rỗi này ra khỏi hàm tối thiểu toàn cục:
    {\large
    \begin{equation}
        \boxed{W_{global} = \min_{P_k \notin \text{Idle}} LW_i(P_k)}
    \end{equation}
    }
    ...
    \item \textbf{Tái phân bổ động dựa trên hết hạn nhịp tim (Heartbeat Timeout Failover)}: Nếu một Worker bị sập vật lý hoặc bị phân mảnh mạng nghiêm trọng (trở thành trạm chậm vĩnh viễn), Coordinator Leader sẽ phát hiện sau khi hết hạn nhịp tim (ngưỡng 10 giây). Hệ thống sẽ kích hoạt giao thức tái phân bổ động, thu hồi quyền sở hữu phân mảnh của node lỗi, tăng nhiệm kỳ (term) của Fencing Token để phong tỏa các lệnh cũ, và phân phối đều các phân mảnh bị ảnh hưởng cho các Worker còn sống gánh hộ. Worker mới nạp trạng thái từ checkpoint Tier-2 để tiếp tục tiêu thụ dữ liệu, giải phóng nghẽn dòng chảy watermark.
\end{itemize}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=1.0\textwidth,height=11.0cm,keepaspectratio]{Partition State Machine.pdf}
...
{\large
\begin{equation}
    \boxed{W_{global}(t) = \min_{\forall P_k \in \text{Active}} LW_i(P_k)}
\end{equation}
}
...
\begin{itemize}
    \item \textbf{Strict Coordinator (Raft/ZooKeeper HA)}: Được triển khai thông qua cụm 3 Coordinator chịu lỗi. Hệ thống hỗ trợ bầu chọn Leader qua (1) thuật toán đồng thuận Raft nhúng (Embedded Raft) với nhân bản nhật ký và bầu cử theo nhiệm kỳ, hoặc (2) ZooKeeper Leader Election tranh chấp khóa phân tán và ghi định danh Leader vào một node tạm. Các chỉ thị điều phối kèm cặp (nhiệm kỳ, định danh lệnh) làm Fencing Token ngăn chặn split-brain khi phân mảnh mạng.
    \item \textbf{Heuristic Aggregator (ZooKeeper/File Lock HA)}: Triển khai thông qua cơ chế dự phòng nóng (Active-Standby) được điều phối qua khóa phân tán ZooKeeper hoặc khóa tệp dùng chung (hỗ trợ đa nền tảng). Active ghi nhịp tim mỗi 1s. Nếu nhịp tim mất hoặc quá hạn ($>1.5$\,s), Standby tự động chiếm khóa, khôi phục trạng thái đã lưu bền vững và lên Active ($RTO\le 2$\,s), tối ưu hiệu năng và tính tự trị cục bộ (Site Autonomy).
\end{itemize}
...
\midrule
\endhead
$LW_i(P_k)$ & Local site progress metadata. \\
$W_{global}$ & Global consistency metadata. \\
...
\begin{figure}[H]
    \centering
    \includegraphics[width=1.0\textwidth,height=11.0cm,keepaspectratio]{domain_4.2.pdf}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=1.0\textwidth,height=13.0cm,keepaspectratio]{Empty Punctuation.pdf}
...
{\large
\begin{equation*}
    \boxed{\forall\, t,\ \forall\, P_k : \quad \big|\,\mathrm{Owner}(P_k,\, t)\,\big| = 1}
\end{equation*}
}
\begin{center}
...
{\large
\begin{equation}
    \boxed{W_{global\_h}(t) = \min_{\forall P_k \in \text{Active}} W_{h,i}^{P_k}(t)}
\end{equation}
}
...
\begin{itemize}
    ...
    \item có thể chạy \textit{Active-Standby} qua khóa phân tán ZooKeeper hoặc File Lock dùng chung,
    \item tự động phục hồi và tiếp quản trạng thái (failover promotion) từ RocksDB/JSON,
...
Sơ đồ điều phối Heuristic: Aggregator (Active-Standby qua khóa ZooKeeper hoặc File Lock dùng chung) gom \texttt{W\_h} từ các Worker tự trị mà không chặn dòng xử lý:
\begin{figure}[H]
    \centering
    \includegraphics[width=1.0\textwidth,height=11.0cm,keepaspectratio]{Heuristic Aggregator.pdf}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=1.0\textwidth,height=13.0cm,keepaspectratio]{Eventual_Consistency_DLQ_correction.pdf}
...
\begin{itemize}
    \item \textbf{Khi xảy ra phân mảnh mạng (P $\rightarrow$ C)}: Hệ thống chọn tính nhất quán (\textbf{C}) và chấp nhận hy sinh tính khả dụng (\textbf{A}) một cách có giới hạn. Khi một Worker bị cô lập mạng hoặc mất kết nối, Coordinator Leader không còn nhận được heartbeat và tiến độ cục bộ của phân mảnh đó. Theo thuật toán tối thiểu toàn cục $W_{global} = \min(LW_i)$, các phân mảnh chưa được xác nhận lỗi vẫn nằm trong hàm $\min()$ để bảo toàn cam kết 0\% loss, do đó watermark toàn cục tạm dừng tịnh tiến và các cửa sổ liên quan bị chặn chốt --- hệ thống hy sinh tính khả dụng trong \emph{cửa sổ phát hiện lỗi}. Sau khi hết hạn nhịp tim, cơ chế failover tái phân bổ các phân mảnh bị ảnh hưởng cho các Worker còn sống và khôi phục tiến trình watermark, trong khi Fencing Token (gắn nhiệm kỳ đơn điệu tăng) ngăn chặn split-brain để bảo đảm tính nhất quán không bị vi phạm. Như vậy, trong suốt giai đoạn phân mảnh mạng hệ thống nhất quán ưu tiên Consistency hơn Availability, đánh đổi khả dụng tạm thời để không bao giờ phát ra dữ liệu thiếu sót.
...
\section{Đặc tả Giao tiếp Phân tán \& Biện minh theo Lý thuyết (Distributed Communication \& Directory Justification)}

Để quản lý trạng thái phân mảnh và điều phối mốc thời gian logic toàn cụm, hệ thống thiết lập một hệ thống giao tiếp đa tầng hoàn chỉnh. Hệ thống giao tiếp này không chỉ thực thi các đường dẫn dữ liệu vật lý mà còn hiện thực hóa trực tiếp các khái niệm lý thuyết cốt lõi của Özsu và Valduriez về \textbf{Distributed Directory (Catalog) Management} (Quản lý Danh bạ/Thư mục Phân tán) và \textbf{Distribution Transparency} (Tính minh bạch Phân tán).

\subsection{gRPC Interface làm Giao thức Quản lý Danh bạ Phân tán}
Özsu và Valduriez chỉ ra rằng một hệ quản trị dữ liệu phân tán cần một bộ quản lý danh bạ để lưu trữ cấu trúc phân mảnh, vị trí lưu trữ thực tế và trạng thái của các site. Lớp giao tiếp gRPC của hệ thống đóng vai trò là giao thức truyền thông của bộ quản lý danh bạ này:

\begin{itemize}[leftmargin=*]
    \item \textbf{Cập nhật Danh bạ Trạng thái (State Directory Updates)}: Thông qua thông điệp nhịp tim, các Worker báo cáo định kỳ danh sách phân mảnh đang kiểm soát kèm theo tiến độ thời gian cục bộ ($LW_i$) và danh sách phân mảnh nhàn rỗi. Đây chính là cơ chế đồng bộ danh bạ động từ các site cục bộ về node điều phối trung tâm.
    \item \textbf{Nhân bản Danh bạ chịu lỗi (Replicated Directory Consistency)}: Bằng cách tích hợp giao thức bầu chọn và nhân bản trạng thái theo đồng thuận Raft, cụm Coordinator duy trì một bản sao danh bạ nhất quán mạnh (Strong Consistency), loại bỏ điểm lỗi đơn lẻ (SPOF) nhưng vẫn đảm bảo tính đúng đắn của dữ liệu metadata phân tán.
    \item \textbf{Tự trị Cục bộ Bất đồng bộ (Asynchronous Site Autonomy)}: Trong chế độ Heuristic, các Worker tự trị gửi mốc Watermark thích ứng $W_h$ cục bộ của từng phân mảnh lên Aggregator một cách bất đồng bộ qua gRPC, giảm thiểu tối đa chi phí chặn truyền thông (non-blocking communication cost).
\end{itemize}

\subsection{HTTP REST APIs làm Giao diện Minh bạch Phân tán (Distribution Transparency)}
Một hệ thống dữ liệu phân tán lý tưởng phải che giấu sự phức tạp của việc phân mảnh và phân bổ dữ liệu khỏi ứng dụng phía trên (dashboard giám sát hoặc client tiêu thụ). Giao diện HTTP REST giám sát và điều khiển của hệ thống hiện thực hóa các mức độ minh bạch này:

\begin{enumerate}
    \item \textbf{Minh bạch Phân mảnh (Fragmentation Transparency)}: Client truy vấn trạng thái toàn cục để lấy mốc thời gian Watermark và danh sách phân mảnh mà không cần biết dữ liệu được phân chia ngang thành 12 phân mảnh logic.
    \item \textbf{Minh bạch Vị trí (Location Transparency)}: Client truy cập các chỉ số đo lường hoặc thông tin chuyển giao phân mảnh từ bất kỳ node nào trong hệ thống mà không cần biết phân mảnh $P_k$ đang nằm vật lý tại Worker nào.
    \item \textbf{Quản lý Danh mục Lược đồ Phân tán (Distributed Schema Catalog)}: Bộ đăng ký lược đồ tập trung (Schema Registry) đảm bảo tính nhất quán của định dạng bản ghi trên toàn cụm phân tán mà không bắt các Worker phải lưu trữ bản sao lược đồ cứng.
\end{enumerate}
