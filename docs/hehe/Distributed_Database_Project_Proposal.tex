...
\begin{itemize}[leftmargin=*]
    ...
    \begin{enumerate}
        \item \textit{Định thời nghiêm ngặt (Strict Watermark)}: Cam kết \textbf{không mất dữ liệu (0\% loss)} trên đường xử lý chính bằng cách chỉ tiến mốc thời gian toàn cục khi \emph{mọi} phân mảnh đang hoạt động đã vượt qua mốc đó (xác nhận qua Punctuation Token từ nguồn). Theo phân loại của Özsu \& Valduriez, đây là thiết kế \textbf{PC/EC} --- ưu tiên tính nhất quán (Consistency) cả khi mạng phân tách lẫn lúc bình thường --- nên chấp nhận chi phí điều phối cao và độ trễ tăng, đồng thời chịu ảnh hưởng bởi hiện tượng nút cổ chai do các phân mảnh chậm (Stragglers).
        \item \textit{Định thời thích ứng thống kê (Heuristic Watermark)}: Cam kết \textbf{độ trễ thấp}, trao quyền tự trị cục bộ tối đa cho Worker (\emph{Site Autonomy}). Mỗi Worker tự ước lượng phân vị độ trễ thực nghiệm bằng cấu trúc DDSketch để đóng cửa sổ sớm mà không chờ phân mảnh khác, đồng thời định tuyến dữ liệu đến muộn sang Hàng đợi xử lý muộn (Dead-Letter Queue - DLQ) để đền bù bất đồng bộ. Đây là thiết kế \textbf{PA/EL} --- ưu tiên tính sẵn sàng (Availability) và độ trễ (Latency) --- chấp nhận nhất quán sau cùng (Eventual Consistency).
    \end{enumerate}
...
\begin{itemize}[leftmargin=*]
    ...
    Kỹ thuật này nén toàn bộ 31 ngày dữ liệu thực tế xuống còn \textbf{12.4 giờ} thời gian logic mô phỏng (giảm 60 lần), trong khi vẫn giữ nguyên các phân phối bất tuần tự đặc trưng (tỷ lệ giảm theo đúng hệ số $\text{DIV}=60$):
    \begin{itemize}[label={-}]
        \item \textbf{Tỷ lệ bất tuần tự (out-of-order)}: 49.41\% sự kiện (1,463,199 bản ghi) đến lệch thứ tự so với sự kiện liền kề trước đó --- đủ khốc liệt để kiểm thử cơ chế Watermark với cửa sổ 5 giây.
        \item Phân vị độ trễ mô phỏng: p50 = \textbf{11.63 s}, p90 = \textbf{28.80 s}, p95 = \textbf{37.78 s}, p99 = \textbf{59.70 s}; trễ cực đại (max lateness) = \textbf{359.73 s}.
    \end{itemize}
    Nếu phát lại dữ liệu theo tỷ lệ 1:1, độ trễ đến muộn gần như bằng 0 so với quãng 31 ngày, khiến đường cong hoàn thiện luôn phẳng ở 100\% và không kiểm thử được Watermark. Cơ chế nén giữ nguyên hành vi bất tuần tự thực tế nhưng cho phép Worker thu thập chỉ số đánh đổi (completeness vs wait time) trong thời gian khả thi.
...
\begin{figure}[H]
    \centering
    \includegraphics[width=1.0\textwidth,height=20.0cm,keepaspectratio]{domain_4.1.pdf}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=1.0\textwidth,height=20.0cm,keepaspectratio]{domain_4.2.pdf}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=1.0\textwidth,height=20.0cm,keepaspectratio]{domain_4.3.pdf}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=1.0\textwidth,height=20.0cm,keepaspectratio]{domain_4.4.pdf}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=1.0\textwidth,height=20.0cm,keepaspectratio]{domain_4.5.pdf}
...
\texttt{KAFKA} & Data Plane & Broker điều phối thông điệp, lưu trữ 12 phân mảnh cho các dòng sự kiện đầu vào, dòng kết quả chính và dòng kiểm toán. \\
...
\begin{figure}[H]
    \centering
    \includegraphics[width=1.0\textwidth,height=21.0cm,keepaspectratio]{Empty Punctuation.pdf}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=1.0\textwidth,height=20.0cm,keepaspectratio]{DDSketch_DLQ_routing.pdf}
...
Chúng tôi đã thực hiện chạy quét thực nghiệm (benchmark sweeps) trên toàn bộ tập dữ liệu \textbf{2,961,423 bản ghi}, áp dụng cơ chế nén thời gian $\text{DIV}=60$ và phân phối độ trễ đã mô tả ở Mục~\ref{sec:dataset} (Dataset Specification). Dưới đây là các phân tích chuyên sâu về sự đánh đổi này.
...
\begin{enumerate}
        ...
        \begin{figure}[H]
            \centering
            \includegraphics[width=1.0\textwidth,height=20.0cm,keepaspectratio]{Partition State Machine.pdf}
        ...
        \begin{figure}[H]
            \centering
            \includegraphics[width=1.0\textwidth,height=20.0cm,keepaspectratio]{5-Step Failback Sequence Diagram.pdf}
