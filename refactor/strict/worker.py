...
from refactor.common.types import LogEvent, PunctuationToken, WorkerHeartbeat
...
class StrictWorker:
    ...
    def __init__(
        ...
        for pid in partition_ids:
            ...
            self.engines[pid] = StrictWatermarkEngine(
                window_size_s=window_size_s,
                delta_base_s=delta_base_s,
                max_queue=max_queue,
                checkpoint_dir=f"/tmp/strict-worker-{worker_id}-p{pid}",
    ...
    def process(self, event: LogEvent, partition_id: int) -> float | None:
        ...
        # Backpressure: reject if this partition is paused
