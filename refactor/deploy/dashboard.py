...
Run:
    pip install -r refactor/deploy/requirements-dashboard.txt
    streamlit run refactor/deploy/dashboard.py
"""
...
def compose_up(mode: str, dataset_file: str) -> tuple[bool, str]:
    ...
    try:
        r = subprocess.run(cmd, capture_output=True, text=True, env=env,
                           cwd=str(DEPLOY_DIR), timeout=600)
...
def compose_down() -> tuple[bool, str]:
    ...
    try:
        r = subprocess.run(cmd, capture_output=True, text=True,
                           cwd=str(DEPLOY_DIR), timeout=120)
        return r.returncode == 0, r.stdout + r.stderr
    except Exception as e:
        return False, str(e)
