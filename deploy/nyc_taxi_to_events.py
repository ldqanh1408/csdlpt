...
Chuẩn hóa dataset/yellow_tripdata_2024-01.csv (19 cột thô) sang schema engine:
...
Nén thời gian — 1 hệ số DIV duy nhất (xem plan):
    ...
    DIV=60 → lateness p50≈12s / p95≈38s / p99≈60s  → δ quét 0..60s
...
Usage:
    python deploy/nyc_taxi_to_events.py --rows 150000 --div 60
    python deploy/nyc_taxi_to_events.py                       # toàn bộ dataset
    python deploy/nyc_taxi_to_events.py --late-target-s 40    # tự tính DIV từ p95
"""
...
DEFAULT_OUT = ROOT / "dataset" / "nyc_taxi_events.csv"
...
def main():
    ...
    t0 = time.time()
    print(f"[converter] reading {args.src} ...", flush=True)
    cols = ["tpep_pickup_datetime", "tpep_dropoff_datetime",
            "PULocationID", "DOLocationID", "total_amount"]
    nrows = args.rows if args.rows > 0 else None
    df = pd.read_csv(args.src, usecols=cols, nrows=nrows,
    ...
    # Use Timedelta.total_seconds() (resolution-safe across pandas datetime units)
    # rather than astype(int64)/1e9 which assumes nanosecond resolution.
    pmin_ts = pu.min()
    ...
    # ── Step 6: Write (index=True → unnamed first col, matches project schema) ──
    out = Path(args.out)
    ...
    print("  python deploy/experiment_completeness_vs_wait.py --mode strict "
          "--punctuation max-event-time --dataset nyc_taxi_events.csv "
          "--deltas 0,5,10,20,40,60 --max-wait 200")
